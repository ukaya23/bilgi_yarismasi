...
class PostgresDatabase {
    constructor() {
        ...
        this.pool.on('error', (err) => {
            logger_1.default.error({ err }, 'Unexpected error on idle DB client');
        });
    }
    async initialize() {
        try {
            ...
            try {
                ...
                logger_1.default.info({ connectedAt: result.rows[0].now }, 'PostgreSQL connected');
            }
            finally {
                client.release();
            }
            ...
            logger_1.default.fatal({ err: error }, 'Database connection error');
            throw error;
        }
    }
    async close() {
    ...
    async deleteQuestion(id) {
        const result = await this.pool.query('UPDATE questions SET is_active = false WHERE id = $1', [id]);
        return result.rowCount;
    }
    // ==================== YARIŞMACI İŞLEMLERİ ====================
    ...
    async getContestantById(id) {
        const result = await this.pool.query('SELECT * FROM contestants WHERE id = $1', [id]);
        return result.rows[0] || null;
    }
    ...
    async updateContestantSocket(id, socketId) {
        const result = await this.pool.query(`UPDATE contestants SET socket_id = $1, status = 'ONLINE' WHERE id = $2`, [socketId, id]);
        return result.rowCount;
    }
    async updateContestantStatus(id, status) {
        const result = await this.pool.query('UPDATE contestants SET status = $1 WHERE id = $2', [status, id]);
        return result.rowCount;
    }
    async updateContestantScore(id, pointsToAdd) {
        const result = await this.pool.query('UPDATE contestants SET total_score = total_score + $1 WHERE id = $2', [pointsToAdd, id]);
        return result.rowCount;
    }
    async getContestantBySocketId(socketId) {
        const result = await this.pool.query('SELECT * FROM contestants WHERE socket_id = $1', [socketId]);
        return result.rows[0] || null;
    }
        ...
        try {
            ...
            if (competitionId) {
                ...
                try {
                    ...
                    logger_1.default.debug({ message: seqError.message }, 'Sequence reset atlaniyor');
                }
            }
    ...
    async resetAllAccessCodes(competitionId) {
        const result = await this.pool.query('UPDATE access_codes SET is_used = false, session_token = NULL, used_at = NULL WHERE competition_id = $1', [competitionId]);
        return result.rowCount;
    }
    // ==================== CEVAP İŞLEMLERİ ====================
    async saveAnswer(questionId, contestantId, answerText, timeRemaining) {
        ...
        try {
            await client.query('BEGIN');
            const existing = await client.query('SELECT id FROM answers WHERE question_id = $1 AND contestant_id = $2', [questionId, contestantId]);
            if (existing.rows.length > 0) {
                await client.query('ROLLBACK');
                return { success: false, message: 'Bu soru için zaten cevap verilmiş' };
            }
            const result = await client.query(`INSERT INTO answers (question_id, contestant_id, answer_text, time_remaining)
                 VALUES ($1, $2, $3, $4) RETURNING id`, [questionId, contestantId, answerText, timeRemaining]);
            await client.query('COMMIT');
    ...
    async saveAnswersBulk(questionId, contestantIds) {
        ...
        await this.pool.query(`INSERT INTO answers (question_id, contestant_id, answer_text, time_remaining)
             VALUES ${valueClauses.join(', ')}`, params);
    }
    ...
    async getAskedQuestionIds(competitionId = null) {
        ...
        const result = await this.pool.query('SELECT DISTINCT question_id FROM answers');
        return result.rows.map((r) => r.question_id);
    }
    async gradeAnswer(answerId, isCorrect, points) {
        ...
        try {
            await client.query('BEGIN');
            await client.query('UPDATE answers SET is_correct = $1, points_awarded = $2 WHERE id = $3', [isCorrect, points, answerId]);
            if (points > 0) {
                const answer = await client.query('SELECT contestant_id FROM answers WHERE id = $1', [answerId]);
                if (answer.rows.length > 0) {
                    await client.query('UPDATE contestants SET total_score = total_score + $1 WHERE id = $2', [points, answer.rows[0].contestant_id]);
                }
            }
            await client.query('COMMIT');
        }
    ...
    async gradeAnswersBulk(answerIds, isCorrect, points) {
        ...
        try {
            await client.query('BEGIN');
            await client.query('UPDATE answers SET is_correct = $1, points_awarded = $2 WHERE id = ANY($3)', [isCorrect, points, answerIds]);
            if (points > 0) {
                await client.query(`
    ...
    async getRandomQuote() {
        const result = await this.pool.query('SELECT * FROM quotes ORDER BY RANDOM() LIMIT 1');
        return result.rows[0] || null;
    }
    async getAllQuotes() {
    ...
    async getOrCreateSession() {
        let result = await this.pool.query('SELECT * FROM game_sessions ORDER BY id DESC LIMIT 1');
        if (result.rows.length === 0) {
            result = await this.pool.query('INSERT INTO game_sessions (state) VALUES ($1) RETURNING *', ['IDLE']);
        }
    ...
    async ensureDefaultAdmin() {
        const existing = await this.pool.query('SELECT id FROM admin_users WHERE username = $1', ['admin']);
        if (existing.rows.length === 0) {
            const hashedPassword = await bcryptjs_1.default.hash('admin123', 10);
            await this.pool.query('INSERT INTO admin_users (username, password_hash) VALUES ($1, $2)', ['admin', hashedPassword]);
            logger_1.default.warn('Default admin user created (username: admin) - Change the default password immediately!');
        }
    }
    async getAdminByUsername(username) {
        const result = await this.pool.query('SELECT id, username, password_hash FROM admin_users WHERE username = $1', [username]);
        return result.rows[0] || null;
    }
    async updateAdminPassword(adminId, hashedPassword) {
        await this.pool.query('UPDATE admin_users SET password_hash = $1 WHERE id = $2', [hashedPassword, adminId]);
    }
    ...
    async getActiveCompetition() {
        const result = await this.pool.query("SELECT * FROM competitions WHERE status = 'ACTIVE' ORDER BY id DESC LIMIT 1");
        return result.rows[0] || null;
    }
    async getActiveCompetitions() {
        const result = await this.pool.query("SELECT * FROM competitions WHERE status = 'ACTIVE' ORDER BY created_at DESC");
        return result.rows;
    }
    async getCompetitionById(id) {
        const result = await this.pool.query('SELECT * FROM competitions WHERE id = $1', [id]);
        return result.rows[0] || null;
    }
    async updateCompetition(id, updates) {
        const { name, status } = updates;
        const result = await this.pool.query('UPDATE competitions SET name = COALESCE($1, name), status = COALESCE($2, status) WHERE id = $3', [name, status, id]);
        return result.rowCount;
    }
    async updateCompetitionStatus(id, status) {
        const result = await this.pool.query('UPDATE competitions SET status = $1 WHERE id = $2', [status, id]);
        return result.rowCount;
    }
    ...
    async getAccessCodesByCompetition(competitionId) {
        const result = await this.pool.query('SELECT * FROM access_codes WHERE competition_id = $1 ORDER BY role, slot_number', [competitionId]);
        return result.rows;
    }
    // ==================== JWT TOKEN İŞLEMLERİ ====================
    ...
    async isTokenRevoked(tokenId) {
        const result = await this.pool.query('SELECT EXISTS(SELECT 1 FROM revoked_tokens WHERE token_id = $1) as revoked', [tokenId]);
    ...
    async isTokenRevokedOrUserBanned(tokenId, userId, tokenIssuedAt) {
        const tokenCheck = await this.pool.query('SELECT EXISTS(SELECT 1 FROM revoked_tokens WHERE token_id = $1) as revoked', [tokenId]);
    ...
    async updateAccessCodeName(codeId, name) {
        const result = await this.pool.query('UPDATE access_codes SET name = $1 WHERE id = $2', [name, codeId]);
        return result.rowCount;
    }
    async resetAccessCode(codeId) {
        const result = await this.pool.query('UPDATE access_codes SET is_used = false, session_token = NULL, used_at = NULL WHERE id = $1', [codeId]);
        return result.rowCount;
    }
}
